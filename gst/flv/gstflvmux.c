...
static GstStaticPadTemplate audiosink_templ = GST_STATIC_PAD_TEMPLATE ("audio",
    ...
    ("audio/x-adpcm, layout = (string) swf, channels = (int) { 1, 2 }, rate = (int) { 5512, 11025, 22050, 44100 }; "
        ...
        "audio/x-alaw, channels = (int) { 1, 2 }, rate = (int) { 5512, 8000, 9000, 11025, 22050, 44100 }; "
        "audio/x-mulaw, channels = (int) { 1, 2 }, rate = (int) { 5512, 8000, 11025, 22050, 44100 }; "
    ...
    if (gst_structure_get_int (s, "rate", &rate)) {
      if (cpad->audio_codec == 10)
        ...
        cpad->rate = 3;
              ...
              || cpad->audio_codec == 14 || cpad->audio_codec == 7
              || cpad->audio_codec == 8))
        cpad->rate = 0;
